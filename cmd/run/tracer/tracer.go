...
var cachedEventFields []*rpc.EventField
...
func initCachedEventColumns() error {
	...
	for i := 0; i < desc.Fields().Len(); i++ {
		...
		case
			...
			cachedEventFields = append(cachedEventFields, &rpc.EventField{
...
func initTunnel(ctx context.Context, endpoint string) (_ *websocket.Conn, finalErr error) {
	...
	b, err := proto.Marshal(&rpc.TunnelClientHello{EventFields: cachedEventFields})
	if err != nil {
	...
	var serverHello rpc.TunnelServerHello
...
func doInsert(ctx context.Context, conn *websocket.Conn, data []byte) error {
	...
	b, err := proto.Marshal(&rpc.TunnelQuery{
	...
	if res.TunnelQueryId != tunnelQueryID.String() {
		...
		return fmt.Errorf("got result query ID %s, want %s", res.TunnelQueryId, tunnelQueryID.String())
	}

	if res.TunnelError != "" {
		return fmt.Errorf("tunnel error: %s", res.TunnelError)
	}

	if res.ClickhouseError != "" {
		return fmt.Errorf("clickhouse error: %s", res.ClickhouseError)
	}
...
func (b *block) flush(ctx context.Context) error {
	...
	var tunnel rpc.CreateTunnel_Response
	if code, err := rpc.Call(ctx, &tunnel, "/api/CreateTunnel", &rpc.CreateTunnel_Request{Role: rpc.TunnelRole_INSERT}); err != nil {
		return fmt.Errorf("call CreateTunnel: %w", err)
	} else if code != http.StatusOK || tunnel.Error != "" {
		err := fmt.Errorf("CreateTunnel: %s", http.StatusText(code))
		if tunnel.Error != "" {
			err = fmt.Errorf("%w: %s", err, tunnel.Error)
		}
		return err
	}

	conn, err := initTunnel(ctx, tunnel.Endpoint)
	if err != nil {
