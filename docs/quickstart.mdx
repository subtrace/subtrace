...
<AccordionGroup>
  <Accordion title='Error: subtrace: missing SYS_PTRACE capability'>
    Subtrace requires the `SYS_PTRACE` Linux capability to work correctly. If you're using Subtrace inside Docker, add the `--cap-add=SYS_PTRACE` flag to your container startup command:
    ```bash
    docker run --cap-add=SYS_PTRACE my-docker-image:latest
    ```
    See [this](/docs/ptrace) for more details.
  </Accordion>
</AccordionGroup>
